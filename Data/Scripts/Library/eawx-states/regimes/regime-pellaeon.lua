...
return {
    on_enter = function(self, state_context)
        ...
        if self.entry_time <= 5 then
            ...
            local rogriss_found = false
            local obj_rogriss = Find_First_Object("Rogriss_Agonizer")
            if TestValid(obj_rogriss) then
                rogriss_found = true
            elseif TestValid(Find_First_Object("Rogriss_Aurora")) then
                obj_rogriss = Find_First_Object("Rogriss_Aurora")
                rogriss_found = true
            end

            if rogriss_found == true then
            ...
            if rogriss_status == "other" then
                ...
                UnitUtil.SetLockList(self.regime_host_name, {"Rogriss_Dominion_Dummy", "Rogriss_Dominion_Dummy2"})
                if rogriss_status ~= "regime_host" then
                    StoryUtil.SpawnAtSafePlanet("BASTION", self.p_regime_host, self.active_planets, {obj_rogriss})
